...
class PyApp(gtk.Window):
   def __init__(self):
      ...
      fixed.put(combobox3, 195, 265)
      fixed.put(combobox4, 100, 310)
   ...
   def hello(self,widget):
    ...
    with open('pathdir1.txt','r') as file:
         path1 = file.read().replace('\n','')

    with open('pathdir2.txt','r') as file:
         path2 = file.read().replace('\n','')

    command1=r"del {}\\output\\gui_plot.png".format(path1)
    command2=r"del {}\\output\\summary.txt".format(path1)
    command3=r"del {}\\output\\insolation.txt".format(path1)    
    command4=r"del {}\\output\\solar.radiation.ctl".format(path1)
    command5=r"del {}\\output\\solar.radiation".format(path1)
    command6=r"del {}\\output\\radiation.ctl".format(path1)    
    command7=r"del {}\\output\\radiation".format(path1)        
    command8="start /wait dinsol.exe"
    command9="start /wait /unix /usr/bin/Rscript {}/output/.0K/gui_plot.R".format(path2)
    ...
    isExist1 = os.path.exists(summary_file)

    for var in range(1,200):
        isExist1 = os.path.exists(summary_file)    
        if not isExist1: time.sleep(2)
        if isExist1: break

    os.system(command9)

    # Specify path
    guiplot_file="output/gui_plot.png"
    isExist2 = os.path.exists(guiplot_file)

    for var in range(1,200):
        isExist2 = os.path.exists(guiplot_file)    
        if not isExist2: time.sleep(2)
        if isExist2: break
    ...
    class PyApp(gtk.Window):
      ...
      def __init__(self):
          ...
          fixed.put(self.spring2, 146, 340)
          fixed.put(self.spring2month, 192, 340)         
          fixed.put(self.summer2, 120, 360)
          fixed.put(self.summer2bar, 192, 360)
          fixed.put(self.summer2month, 200, 360)
          fixed.put(self.autumn2, 120, 380)
          fixed.put(self.autumn2bar, 192, 380)
          fixed.put(self.autumn2month, 200, 380)
          fixed.put(self.winter2, 120, 400)
          fixed.put(self.winter2bar, 192, 400)
          fixed.put(self.winter2month, 200, 400)
          fixed.put(self.peri2, 120, 430)
          fixed.put(self.peri2bar, 192, 430)
          fixed.put(self.peri2month, 200, 430)
          fixed.put(self.afel2, 120, 450)
          fixed.put(self.afel2bar, 192, 450)
          fixed.put(self.afel2month, 200, 450)
          fixed.put(self.insol12, 130, 480)
          fixed.put(self.insol22, 130, 500)
          fixed.put(self.degr0, 250, 250)
          fixed.put(self.degr1, 250, 270)
          fixed.put(self.watts0, 250, 130)
          fixed.put(self.watts1, 250, 480)
          fixed.put(self.watts2, 250, 500)
